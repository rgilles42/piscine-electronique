...
void	uart_init(uint32_t baudrate) {
	uint32_t ubbr_value = (F_CPU / 8 / baudrate - 1) / 2;	// witout the -1 because the integer logic truncates instead of rounding
	...
	UCSR0B = _BV(TXEN0) | _BV(RXEN0) | _BV(RXCIE0);			// Enable UART transmission, reception, and interrupt on receive.
}
